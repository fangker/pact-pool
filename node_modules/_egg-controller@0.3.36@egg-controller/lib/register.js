...
function registerRoute(app) {
    ...
    const routeDatas = {};
    controller_1.getControllers(app.config).forEach(ctrl => {
        ctrl.routes
            .sort((a, b) => {
            return a.url > b.url ? -1 : 1;
        })
            .forEach(route => {
            const routeData = (routeDatas[route.typeGlobalName] =
    ...
    fs.writeFileSync(path.join(app.baseDir, 'run', 'route_map.json'), JSON.stringify(routeDatas, null, 2), { encoding: 'utf8' });
    const pkg = app.config.pkg;
    const openAPIInfo = openapi_1.convertToOpenAPI({
        base: {
    ...
    }, controller_1.getRoutes(app.config));
    fs.writeFileSync(path.join(app.baseDir, 'run', 'openapi_3.json'), JSON.stringify(openAPIInfo, null, 2), { encoding: 'utf8' });
